...
embedding2 = CohereEmbeddings(model="embed-english-light-v3.0", cohere_api_key="OAmsKBS62dhY5JlrMPqIgGq5mqUvRp8J6KLpnWUj")
...
def chat_with_document(request, file_id):
    ...
    if request.method == 'POST':
        ...
        compressed_docs = compression_retriever.get_relevant_documents(question)
        prompt = hub.pull("qa1/chat_with_doc", api_url="https://api.hub.langchain.com", api_key='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')
...
def summarize_page(file_obj, page_number):
    ...
    Keep the summary as concise as possible and easy to read without changing orignal meaning oc content. 
    summarize each paragragh. format your text properly and use bullet points format to summarize paragraphs and put each summary under the appropriate heading or subheading. 
    ...
    """
    QA_CHAIN_PROMPT = hub.pull("qa1/summarize", api_url="https://api.hub.langchain.com", api_key='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')
    ...
    question="only summarize the context given. dont answer any quesions in the context"
    result = qa({"query": question})
        ...
        try:
            ...
            if question:
                try:
                    ...
                    response_generator = generate_response(conversation_history, request)  # Pass the request object
...
def generate_response(conversation_history, request):
